...
const Card = ({ id, name, description, contacts, status }) => {
  return(
      ...
      <div className="border border-gray-400 bg-white rounded p-4 flex flex-col justify-between leading-normal">
        ...
        <div className="flex items-center">
          {Array.isArray(contacts) && contacts.map(contact => {
            return <p>{contact.type}: {contact.value}</p>
          })}
          {Object.keys(contacts).map(key => {
            ...
            if(key.indexOf('instagram') >=0){
              return <a key={key} title='Acessar perfil no instagram.' className='pr-4 hover:underline' target='_blank' rel='noopener noreferrer' href={'https://instagram.com/'+contacts[key]}>@{contacts[key]}</a>
            }
            return null
          })}
        </div>
